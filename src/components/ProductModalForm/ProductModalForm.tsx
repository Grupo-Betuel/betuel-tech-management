...
// import {IProductData, IProductParam, ProductParamTypes} from "../../models/productModels";
...
import {IProductData, IProductParam, ProductParamTypes} from "../../models/productModels";
import "./ProductModalForm.scss";
import {addProduct, deleteProductParam, handleProductOffers, updateProducts} from "../../services/productService";
import {ECommerceTypes, getWhatsappMessageURL} from "../../services/promotions";
import CorotosFavicon from "../../assets/images/corotos-favicon.png";
import FleaFavicon from "../../assets/images/flea-favicon.png";
import {PromotionOption} from "../../screens/Dashboard/Dashboard";
...
import {addCategory, getCategories, updateCategories} from "../../services/categoryService";
import {extractNumbersFromText, removeHTMLChars} from "../../utils/text.utils";
import {IProductOffers, ProductOfferTypes} from "../../models/productOfferModels";
...
const ProductModalForm: React.FC<IProductFormProps> = (
    ...
    const [offers, setOffers] = React.useState<IProductOffers[]>(editProduct?.productOffers || []);
    const [currentOffer, setCurrentOffer] = React.useState<Partial<IProductOffers>>({});
    const [isEditingOffer, setIsEditingOffer] = React.useState<boolean>(false);
    ...
    // Offer handling
    const onChangeOffer = (ev: React.ChangeEvent<any>) => {
        const {name, value, type, checked} = ev.target;
        const finalValue = type === "checkbox" ? checked : value;
        setCurrentOffer({...currentOffer, [name]: finalValue});
    }

    const addOrUpdateOffer = async () => {
        try {
            let newOffers: IProductOffers[] = []
            if (product._id) {
                setIsSubmiting(true)
                if (isEditingOffer) {
                    newOffers = offers.map(offer => (offer.tempIndex || offer._id) === (currentOffer.tempIndex || currentOffer._id) ? currentOffer : offer) as IProductOffers[]
                } else {
                    newOffers = [...offers, {
                        ...(currentOffer as IProductOffers),
                        tempIndex: new Date().getTime().toString()
                    }]
                }

                setCurrentOffer({});
                setIsEditingOffer(false);
                newOffers = await handleProductOffers({productId: product._id, offers: newOffers});
                setProduct({
                    ...product,
                    productOffers: newOffers
                });
            ...
            console.error(e);
            toast('Error al agregar oferta', {type: 'error'});
            setIsSubmiting(false)
        }
    }

    const editOffer = (offer: IProductOffers) => {
        setCurrentOffer(offer);
        setIsEditingOffer(true);
    }

    const disableOffer = (offerId: string) => {
        setOffers(offers.map(offer => offer._id === offerId ? {...offer, enabled: false} : offer));
    }
        ...
        <Modal
            ...
            <Form className="product-form">
                ...
                <ModalBody style={{display: (enableFlyer && editProduct) ? 'none' : ''}}>
                    ...
                    {/* Offers Section */}
                    {product._id && <FormGroup>
                        <Label><b>Ofertas</b></Label> <br/>
                        <div className="d-flex w-100 flex-column gap-3">
                            <Input
                                type="select"
                                className="w-100"
                            ...
                            >
                                ...
                                {Object.values(ProductOfferTypes).map((type) => (
                                    <option value={type} key={type}>{type}</option>
                                ))}
                            </Input>
                            {currentOffer.type === ProductOfferTypes.DueDate && (
                                <>
                                    <Input
                                        type="date"
                                        name="startDate"
                                        value={currentOffer.dueDate?.startDate || ''}
                                        onChange={onChangeOffer}
                                    />
                                    <Input
                                        type="date"
                                        name="endDate"
                                        value={currentOffer.dueDate?.endDate || ''}
                                        onChange={onChangeOffer}
                                    />
                                </>
                            )}
                            {currentOffer.type === ProductOfferTypes.Volume && (
                                <>
                                    <Input
                                        type="number"
                                        name="buy"
                                        value={currentOffer.volume?.buy || ''}
                                        onChange={onChangeOffer}
                                    ...
                                    <Input
                                        type="number"
                                        name="get"
                                        value={currentOffer.volume?.get || ''}
                                        onChange={onChangeOffer}
                                        placeholder="Get quantity"
                                    />
                                </>
                            )}
                            {currentOffer.type === ProductOfferTypes.Stock && (
                                <Input
                                    type="number"
                                    name="stock"
                                    value={currentOffer.stock || ''}
                                    onChange={onChangeOffer}
                            ...
                            />
                            <Button color="success" onClick={addOrUpdateOffer} outline>
                                {isEditingOffer ? 'Actualizar Oferta' : 'Agregar Oferta'}
                            </Button>
                        </div>

                        <div className="d-flex flex-wrap gap-2 align-items-center">
                            {offers.map((offer, i) => (
                                <Card key={`${offer.type}-${i}`}
                                       className="d-flex align-items-center gap-2 p-2 cursor-no-pointer"
                                >
                                    <CardBody>
                                        {offer.type}
                                        <i className="cursor-pointer bi bi-x-circle-fill tex-white cursor-pointer font-weight-bold"
                                           onClick={() => disableOffer(offer.tempIndex as string)}/>
                                        <i className="cursor-pointer bi bi-pencil-fill tex-white cursor-pointer font-weight-bold"
                ...
                <ModalFooter>
                    <Button color="danger" onClick={toggleModal} outline>Salir</Button>{' '}
                    <Button color="success" disabled={!product.category}
                            onClick={() => onSubmit(undefined, undefined, true)} outline>Guardar</Button>{' '}
                </ModalFooter>
            </Form>
            <Modal isOpen={productParamToDelete !== ''} toggle={resetProductParam}>
                <ModalHeader toggle={resetProductParam}>Confirmación</ModalHeader>
