...
export const ClientMng = ({
    ...
    const fromClients: { [N in OrderFromTypes]: number } = useMemo(() => {
    ...
    return (
        <div className="clients-mng">
            <div className="d-flex flex-wrap gap-4 pt-4 pb-5">
                {
                    Object.keys(fromClients).map((key, index) => (
                        <div key={index} className="d-flex align-items-center">
                            <b>{key}</b>
                            <span className="ms-3">{(fromClients as any)[key]}</span>
                        </div>
                    ))}
            </div>
